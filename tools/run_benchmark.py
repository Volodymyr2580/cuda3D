...
CASE_DEFAULTS = {
    ...
    "perf_1gpu": {
        "case_dir": "benchmarks/cases/perf_1gpu",
        "input": "input_perf_1gpu.in",
        "np": 1,
        "gpus": "0",
        "timeout": 1800,
    },
    "perf_1gpu_6shots": {
        ...
        "input": "input_perf_1gpu_6shots.in",
        "np": 1,
        "gpus": "0",
        "timeout": 2400,
    },
...
def main():
    ...
    parser.add_argument("--input", help="Override the default input file name inside the case directory.")
    ...
    input_name = args.input or defaults["input"]
    ...
    mpi_home = os.environ.get("MPI_HOME", "/opt/intel/oneapi/mpi/latest")
    env_info["tool_versions"] = run_capture(
        f"{cuda_home}/bin/nvcc --version; {mpi_home}/bin/mpirun --version | head -5; gcc --version | head -1",
        root,
    )

    mpi_env_flags = []
    for key in ["CUDA3D_PML_DUMP_DIR", "CUDA3D_PML_DUMP_STEP"]:
        value = os.environ.get(key)
        if value:
            mpi_env_flags.extend(["-genv", key, value])
    mpi_env = " ".join(shlex.quote(part) for part in mpi_env_flags)
    if mpi_env:
        env_info["mpi_env"] = mpi_env_flags

    cmd = (
        "source /opt/intel/oneapi/setvars.sh >/dev/null 2>&1; "
        f"cd {case_dir}; "
        f"CUDA_VISIBLE_DEVICES={gpus} timeout {timeout}s "
        f"{mpi_home}/bin/mpirun -np {mpi_np} {mpi_env} "
