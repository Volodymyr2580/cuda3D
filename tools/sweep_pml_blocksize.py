#!/usr/bin/env python3
"""Compile and benchmark PML-only CUDA block-size variants."""
...
def main() -> int:
    ...
    report_dir = root / "benchmarks" / "reports" / f"pml_blocksize_sweep_{timestamp}"
    report_dir.mkdir(parents=True, exist_ok=False)

    records = []
    for b1, b2, b3 in args.variants:
        tag = f"pml_{b1}x{b2}x{b3}"
        nvflags = (
            f"-O3 -arch={args.nv_arch} -D_gpu_par_ -Dradius=4 "
            "-DBlockSize1=128 -DBlockSize2=2 -DBlockSize3=1 "
            "-DVBlockSize1=128 -DVBlockSize2=2 -DVBlockSize3=1 "
    ...
    lines = ["# PML Block Size Sweep", "", "| Variant | Threads | WP seconds | Gradient seconds | Run dir |", "|---|---:|---:|---:|---|"]
